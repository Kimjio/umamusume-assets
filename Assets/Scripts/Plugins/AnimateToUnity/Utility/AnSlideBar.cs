...
{
	public class AnSlideBar : AnUIBase
	{
		...
		protected override bool _InitializeThisData()
		{
			...
			{
				anMotion = this._root.Find<AnMotion>(this._rootObject, this._moveButtonMotionPath, false);
			}
			if (anMotion == null)
			{
				return false;
			...
			this._moveButton.FlActionDownInStart.AddAction(new Action<object>(this._OnDownInSliderButton), 0, -1);
			this._moveButton.FlActionDownInLoop.AddAction(new Action<object>(this._OnDownLoopSliderButton), 0, -1);
			this._moveButton.FlActionDownLoop.AddAction(new Action<object>(this._OnDownLoopSliderButton), 0, -1);
			this._moveButton.FlActionDownOutStart.AddAction(new Action<object>(this._OnDownOutSliderButton), 0, -1);
		...
		protected override void _UpdateValueChange()
		{
			...
			if (this._animationFlag)
			{
				...
				float num = 0f;
				float num2 = 0f;
				this._FixSliderValue(this._blendValue.CurrentValue, out num, out num2);
				this._barMotion.SetMotionPause((this._barMotion.CurrentLabelTimeLength - this._barMotion.Root.Parameter.OneFrameTime) * num2);
			...
			else
			{
				...
				float num3 = 0f;
				this._FixSliderValue(this._value, out this._value, out num3);
				this._barMotion.SetMotionPause((this._barMotion.CurrentLabelTimeLength - this._barMotion.Root.Parameter.OneFrameTime) * num3);
			}
		}
		...
		private void _FixSliderValue(float inputValue, out float outputValue, out float outputValuePercent)
		{
			...
			AnUtilityValue.LimitValue(ref outputValue, this._minValue, this._maxValue);
			if (this._maxValue - this._minValue > 0f)
			{
				outputValuePercent = (outputValue - this._minValue) / (this._maxValue - this._minValue);
			}
			else
			{
			...
			AnUtilityValue.LimitValue(ref outputValuePercent, 0f, 1f);
		...
		private void _UpdateRangePosition()
		{
			...
			this._startPosition = this._moveButton.Motion.GameObject.transform.position;
			this._barMotion.SetMotionPause(this._barMotion.CurrentLabelTimeLength);
			this._endPosition = this._moveButton.Motion.GameObject.transform.position;
		...
		protected override void _ResetPrevValue()
		{
			...
			this._prevValue = float.MinValue;
		}
		...
		public override bool _UpdateUI(object arg)
		{
			...
			if (this._stepCount == 0)
			{
				num = (this._maxValue - this._minValue) / 10f;
			}
			else
			{
				num = (this._maxValue - this._minValue) / (float)this._stepCount;
			}
